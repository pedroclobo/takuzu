...
def sequence(iterable):
	...
	return iterable if isinstance(
	    iterable, collections.abc.Sequence) else tuple([iterable])
...
def cross_entropy_loss(x, y):
	return (-1.0 / len(x)) * sum(_x * np.log(_y) + (1 - _x) * np.log(1 - _y)
...
def gaussian(mean, st_dev, x):
	...
	return 1 / (np.sqrt(2 * np.pi) *
	            st_dev) * np.e**(-0.5 * (float(x - mean) / st_dev)**2)
...
def expr(x):
	...
	return eval(expr_handle_infix_ops(x),
	            defaultkeydict(Symbol)) if isinstance(x, str) else x
